...
describe Provet::ConsultationItemTemplate do
  let(:response) { JSON.parse(load_fixture('consultation_item_templates', 'show')) }
  let(:body) { spy }
  let(:consultation_id) { '1' }

  describe '#list' do
    let(:response) { JSON.parse(load_fixture('consultation_item_templates', 'index')) }
    ...
    it 'sends a GET request to fetch the consultation item template list' do
      expect(described_class)
        .to receive(:get)
        .once
        .with('/consultationitemtemplate/', headers: headers, query: {})
        .and_return(response)
  ...
  describe '#all' do
    ...
    it 'sends a GET request to fetch consultation item templates for each page' do
      (1..2).each do |page|
        stub_request(
          :get,
          File.join(
        ...
        expect(described_class)
          .to receive(:get)
          .once
          .with('/consultationitemtemplate/', headers: headers, query: { page: page })
          .and_return(response)
      end
  ...
  describe '#find' do
    ...
    it 'sends a GET request to fetch the consultation item template' do
      expect(described_class)
        .to receive(:get)
        .once
        .with('/consultationitemtemplate/123/', headers: headers, query: {}).and_return(response)
