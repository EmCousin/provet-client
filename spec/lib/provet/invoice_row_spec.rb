...
describe Provet::InvoiceRow do
  let(:response) { JSON.parse(load_fixture('invoice_rows', 'show')) }
  let(:body) { spy }

  describe '#list' do
    let(:response) { JSON.parse(load_fixture('invoice_rows', 'index')) }
    ...
    it 'sends a GET request to fetch the invoice row list' do
      expect(described_class)
        .to receive(:get)
        .once
        .with('/invoicerow/', headers: headers, query: {})
        .and_return(response)
  ...
  describe '#all' do
    ...
    it 'sends a GET request to fetch invoice rows for each page' do
      (1..2).each do |page|
        stub_request(
          :get,
          File.join(
        ...
        expect(described_class)
          .to receive(:get)
          .once
          .with('/invoicerow/', headers: headers, query: { page: page })
          .and_return(response)
      end
  ...
  describe '#find' do
    ...
    it 'sends a GET request to fetch the invoice row' do
      expect(described_class)
        .to receive(:get)
        .once
        .with('/invoicerow/123/', headers: headers, query: {}).and_return(response)
